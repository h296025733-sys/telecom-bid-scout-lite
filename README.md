...
围绕语音线路、短信通道、呼叫中心、运营商专线等业务方向，项目提供一套轻量规则筛选流程，适合用于轻量自动化、人工复核前置、候选公告分级，以及 OpenClaw 搜索结果的后处理。

项目设计说明：

- 为什么用可解释规则先做最小闭环，而不是直接让模型黑盒判断；
- 如何处理低可信转载来源、过期公告和缺少官方入口的风险；
- 如何让 OpenClaw 负责搜索调度，本项目负责稳定规则判断；
- 如何通过状态文件降低重复通知噪音；
